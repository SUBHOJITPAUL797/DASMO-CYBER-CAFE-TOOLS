...
# Must show the new version e.g. 1.5.2.0
```
...
& $wix build `
    ...
    -d "PublishDir=$pubDir" `
    -arch x64 `
    -o "c:\CODING\coading\DASMO CYBER CAFE\releases\DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi"
```

> **Why output directly to `releases/`?** This keeps `releases/` as the single source of truth.
> The MSI filename must include the version number e.g. `_v1.5.3.msi`

After build, verify the MSI:
```powershell
Get-Item "c:\CODING\coading\DASMO CYBER CAFE\releases\DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi" | Select-Object Name, @{N='Size(MB)';E={[math]::Round($_.Length/1MB,1)}}, LastWriteTime
# Expected: ~77 MB
```
...
2. **Install new MSI** — Double-click:
   ```
   releases\DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi
   ```
...
gh release create v1.5.3 `
    ...
    --title "v1.5.3 — [Short Description]" `
    --notes "## What's New in v1.5.3
- Feature 1
- Bug fix 1
- Improvement 1"
```
...
> ```powershell
> gh release upload v1.5.3 "releases\DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi" `
...
# Fields to update:
#   latestVersion:      "1.5.3"
#   minRequiredVersion: "1.5.3"      ← set this ONLY if update is MANDATORY
#   forceUpdate:        true          ← set to true for mandatory updates
#   updateDownloadUrl:  "https://github.com/SUBHOJITPAUL797/DASMO-CYBER-CAFE-TOOLS/releases/download/v1.5.3/DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi"
```
...
|---------------------|---------|------|--------|
| `DASMO_CYBER_CAFE_TOOLS_Setup_v1.5.3.msi` | 1.5.3.0 | 2026-09-22 | ✅ Current |
